...
class Event{
public:
  float thetaS,phiS,raS,decS,thetaTrSubV,costhetaTrSubV,phiTrSubV,thetaTrSubN,costhetaTrSubN,betaTrSubN,losPathLen,thetaNSubV,costhetaNSubV;
  
  Event(): thetaS(0), phiS(0), raS(0), decS(0), thetaTrSubV(0), costhetaTrSubV(1.), phiTrSubV(0), thetaTrSubN(0), costhetaTrSubN(1.), betaTrSubN(0), losPathLen(0), thetaNSubV(0), costhetaNSubV(1.){};
  Event(float ranthetaS, float ranphiS, float ranraS, float randecS, float ranthetaTrSubV, float cosranthetaTrSubV, float ranphiTrSubV, float ranthetaTrSubN, float cosranthetaTrSubN, float ranbetaTrSubN, float ranlosPathLen, float ranthetaNSubV, float cosranthetaNSubV): thetaS(ranthetaS), phiS(ranphiS), raS(ranraS), decS(randecS), thetaTrSubV(ranthetaTrSubV), costhetaTrSubV(cosranthetaTrSubV), phiTrSubV(ranphiTrSubV), thetaTrSubN(ranthetaTrSubN), costhetaTrSubN(cosranthetaTrSubN), betaTrSubN(ranbetaTrSubN), losPathLen(ranlosPathLen), thetaNSubV(ranthetaNSubV), costhetaNSubV(cosranthetaNSubV){};
};
...
private:
  ...
  float maxThetaTrSubV; // This is the maximum theta_Ch for optical Cherenkov detection
...
public:
  ...
  Geom_params(float,float,float,float,float,float,float,float,float,float);
  ~Geom_params(){}
...
Geom_params::Geom_params(float radE, float detalt, float detra, float detdec, float delAlpha, float maxsepangle, float delAziAng, float ParamPi)
  :gen(rd()), dis(0.0, nextafter(1.0, std::numeric_limits<float>::max())) // Closed interval [0.0,1.0]
{
  ...
  minThetaS = acos((radEplusDetAltSqrd + earthRadiusSqrd
		    - minLOSpathLenSqrd)*1./(2.*radEplusDetAlt*earthRadius));
  ...
  bracketForNormThetaS = (radEplusDetAltSqrd - earthRadiusSqrd)*maxLOSpathLen
  ...
  mcnorm = earthRadiusSqrd/pdfnorm; // Still need to divide by the number of trajectories
...
Geom_params::Geom_params(float radE, float detalt, float detra, float detdec, float nadmin, float nadmax, float maxsepangle, float minAziAng, float maxAziAng, float ParamPi)
  :gen(rd()), dis(0.0, nextafter(1.0, std::numeric_limits<float>::max())) // Closed interval [0.0,1.0]
{
  float bracketForNormThetaS;
  ...
  minThetaS = acos((radEplusDetAltSqrd + earthRadiusSqrd
		    - minLOSpathLenSqrd)*1./(2.*radEplusDetAlt*earthRadius));
  maxThetaS = acos((radEplusDetAltSqrd + earthRadiusSqrd
		    - maxLOSpathLenSqrd)*1./(2.*radEplusDetAlt*earthRadius));
  cosOfMaxThetaS = cos(maxThetaS);
  cosOfMinThetaS = cos(minThetaS);
...
void Geom_params::gen_traj() {
  ...
  float rthetaS, rphiS, rraS, rdecS, rthetaTrSubV, rcosthetaTrSubV, rphiTrSubV, rthetaTrSubN, rbetaTrSubN, rlosPathLen;
  float u1, u2, u3, u4;
  ...
  // Generate theta_s (the colatitude on the surface of the Earth in the detector nadir perspective)
  ...
  if (discriminant <= 0){
    ...
    if ((v1 > 0)&&(v1 >= minLOSpathLen)&&(v1 <= maxLOSpathLen)) rlosPathLen = v1;
    if ((v2 > 0)&&(v2 >= minLOSpathLen)&&(v2 <= maxLOSpathLen)) rlosPathLen = v2;
    if ((v3 > 0)&&(v3 >= minLOSpathLen)&&(v3 <= maxLOSpathLen)) rlosPathLen = v3;
  }
  else {
  ...
  if (rcosthetaTrSubN < 0.0) geoKeep = false; // Trajectories going into the ground
  ...
  this->localevent = Event(rthetaS, rphiS, rraS, rdecS, rthetaTrSubV, rcosthetaTrSubV, rphiTrSubV, rthetaTrSubN, rcosthetaTrSubN, rbetaTrSubN, rlosPathLen, rthetaNSubV, rcosthetaNSubV);
...
void Geom_params::gen_traj_from_set(float u1, float u2, float u3, float u4){
  ...
  rcosthetaNSubV = (radEplusDetAltSqrd - earthRadiusSqrd - rvsqrd)/(2.*earthRadius*rlosPathLen);
...
void Geom_params::get_local_event(){
  ...
  printf("%f %f %f %f %f %f %f %f %f %f %f %f %f %s\n",e.thetaS,e.phiS,e.raS,e.decS,e.thetaTrSubV,e.costhetaTrSubV,e.phiTrSubV,e.thetaTrSubN,e.costhetaTrSubN,e.betaTrSubN,e.losPathLen,e.thetaNSubV,e.costhetaNSubV,kestring);
}
...
//  const float thetaCh = GeoPi*(1.5/180.); // For the geometry factor for the trajectories only monte carlo.
...
//    if (keepThisEvent)
//      geom_factor += (heaviside(thisevent.costhetaTrSubV - costhetaCh)*thisevent.costhetaTrSubN)
//	/(thisevent.costhetaNSubV*thisevent.costhetaTrSubV);
//  }

//  geom_factor *= (1./numEvents)*mcnorm;

//  // This block is for the attribute that includes the complete event information in the HDF5 File implementation
...
//    evtype.insertMember(MEMBER1, HOFFSET(ev_data_t, ts), PredType::NATIVE_FLOAT);
//    evtype.insertMember(MEMBER2, HOFFSET(ev_data_t, ps), PredType::NATIVE_FLOAT);
//    evtype.insertMember(MEMBER3, HOFFSET(ev_data_t, rs), PredType::NATIVE_FLOAT);
//    evtype.insertMember(MEMBER4, HOFFSET(ev_data_t, ds), PredType::NATIVE_FLOAT);
//    evtype.insertMember(MEMBER5, HOFFSET(ev_data_t, ttsv), PredType::NATIVE_FLOAT);
//    evtype.insertMember(MEMBER6, HOFFSET(ev_data_t, cttsv), PredType::NATIVE_FLOAT);
//    evtype.insertMember(MEMBER7, HOFFSET(ev_data_t, ptsv), PredType::NATIVE_FLOAT);
//    evtype.insertMember(MEMBER8, HOFFSET(ev_data_t, ttsn), PredType::NATIVE_FLOAT);
//    evtype.insertMember(MEMBER9, HOFFSET(ev_data_t, cttsn), PredType::NATIVE_FLOAT);
//    evtype.insertMember(MEMBER10, HOFFSET(ev_data_t, btsn), PredType::NATIVE_FLOAT);
//    evtype.insertMember(MEMBER11, HOFFSET(ev_data_t, lpl), PredType::NATIVE_FLOAT);
//    evtype.insertMember(MEMBER12, HOFFSET(ev_data_t, tnsv), PredType::NATIVE_FLOAT);
//    evtype.insertMember(MEMBER13, HOFFSET(ev_data_t, ctnsv), PredType::NATIVE_FLOAT);
//    evtype.insertMember(MEMBER14, HOFFSET(ev_data_t, ke), boolenumtype);

//    DataSet *dataset = new DataSet(file->createDataSet(DATASET_NAME, evtype, *dataspace));
...
//    hsize_t attr_dims[1]; //= { (int) dims[1] }; // Dimensions of attribute dataset
//    attr_dims[0] = attrstr.size();
...
//    Attribute *attr = new Attribute(dataset->createAttribute(ATTR_NAME,strdatatype,*attr_dataspace));

//    vector<const char *> cStrArray;
//    for (int index = 0; index < attrstr.size(); ++index) cStrArray.push_back(attrstr[index].c_str());
...
void Geom_params::run_geo_dmc_from_num_traj_nparray(int numEvents){
  ...
  // This block is for reading from the numpy array of uniform random floats and for making the event numpy array

  auto result = py::array(py::buffer_info(nullptr, sizeof(Event),
					    py::format_descriptor<Event>::format(),
					    1,
					    {numEvents},
					    {sizeof(Event)}));

  auto resultm = py::array(py::buffer_info(nullptr, sizeof(bool),
  ...
  for (idx = 0; idx < numEvents; idx++){
    ...
    if (keepLocalEv)
      geom_factor += (heaviside(localevent.costhetaTrSubV - costhetaCh)*localevent.costhetaTrSubN)
...
PYBIND11_MODULE(nssgeometry, m) {
  ...
  py::class_<Event>(m, "Event")
    .def(py::init<>())
    .def(py::init<float,float,float,float,float,float,float,float,float,float,float,float,float>(), py::arg("ranthetaS"), py::arg("ranphiS"), py::arg("ranraS"), py::arg("randecS"), py::arg("ranthetaTrSubV"), py::arg("cosranthetaTrSubV"), py::arg("ranphiTrSubV"), py::arg("ranthetaTrSubN"), py::arg("cosranthetaTrSubN"), py::arg("ranbetaTrSubN"), py::arg("ranlosPathLen"), py::arg("ranthetaNSubV"), py::arg("cosranthetaNSubV"))
  ...
  // This block is for the numpy array implementation
    ...
    PYBIND11_NUMPY_DTYPE(Event,thetaS,phiS,raS,decS,thetaTrSubV,costhetaTrSubV,phiTrSubV,thetaTrSubN,costhetaTrSubN,betaTrSubN,losPathLen,thetaNSubV,costhetaNSubV);

  py::class_<Geom_params>(m, "Geom_params")
    .def(py::init<float,float,float,float,float,float,float,float>(), py::arg("radE") = 6371.0, py::arg("detalt") = 525.0, py::arg("detra") = 0.0, py::arg("detdec") = 0.0, py::arg("delAlpha") = 4.*atan(1.)*(7.0/180.0), py::arg("maxsepangle") = 4.*atan(1.)*(3.0/180.), py::arg("delAziAng") = 2.0*4.*atan(1.), py::arg("ParamPi") = 4.*atan(1.))
    .def(py::init<float,float,float,float,float,float,float,float,float,float>(), py::arg("radE")= 6371.0, py::arg("detalt")= 525.0, py::arg("detra") = 0.0, py::arg("detdec") = 0.0, py::arg("nadmin") = 0.0, py::arg("nadmax") = 2.*atan(1.), py::arg("maxsepangle") = 4.*atan(1.)*(3.0/180.), py::arg("minAziAng") = 0.0, py::arg("maxAziAng") = 2.*4.*atan(1.), py::arg("ParamPi") = 4.*atan(1.))    
    ...
    .def("run_geo_dmc_from_num_traj_nparray",&Geom_params::run_geo_dmc_from_num_traj_nparray)
    // .def("run_geo_dmc_from_ran_array_hdf5",&Geom_params::run_geo_dmc_from_ran_array_hdf5)
    .def("run_geo_dmc_from_ran_array_nparray",&Geom_params::run_geo_dmc_from_ran_array_nparray)
    ...
    .def_readwrite("evMasknpArray", &Geom_params::evMasknpArray); // This block is for the numpy array implementation  
}
