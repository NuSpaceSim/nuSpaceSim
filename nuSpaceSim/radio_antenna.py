...
def voltage_from_field(
    ...
    V = 2*Efield * (1e3 * c/(1e6 * freqs)) * np.sqrt((Z_antenna / Z_0) * G/(4*np.pi))
...
def noise_efield(
    ...
    E_noise = np.sqrt(np.sum((2*freqs*1e6/c * np.sqrt(T_comb * bw * k_b * Z_0 * np.pi))**2))
    return E_noise
