...
def voltage_from_field(
    ...
    V = 2*Efield * ((1e3 * c/(1e6 * freqs)) * np.sqrt((Z_antenna / Z_0) * G/(4*np.pi))).reshape((-1,1))
...
def noise_voltage(
    ...
    freqs: np.ndarray
        frequency band in MHz
    N: int
        how many noise realizations to take
    h_obs: float
    ...
    skyFrac = 1. - (solid_angle/(4*np.pi))
    T_earth = 287 * np.ones_like(freqs) #average earth temp in kelvin
    T_sys = 100. * np.ones_like(freqs) #reasonable number, could be whatever
    ...
    V_noise = np.sqrt(T_comb * bw * k_b * Z_load).reshape((-1,1))
    return V_noise
...
def calculate_snr(
    ...
    df = 10. #efields made with 10 MHz bins, would need to redo for different bin size
    freqs = np.arange(freqRange[0], freqRange[1], df) + df/2.
    
    V_sig = Nants * voltage_from_field(Efield, freqs, gain)
    V_noise = np.sqrt(Nants * np.sum(noise_voltage(freqs, V_sig.shape[1], h_obs)**2.0))
    V_sigsum = np.sum(V_sig, axis=0)
    #print(V_sigsum.mean())
    #print(V_noise)
