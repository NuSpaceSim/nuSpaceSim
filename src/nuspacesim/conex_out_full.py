...
def conex_out_full(data,geom):
    ...
    mask=(Zfirst>=0) & (Zfirst<=20)  & (TauEnergy >= 20) & (data["lenDec"]<=100)#& (np.degrees(data["beta_rad"])>=5)
    ...
    Xfirst=Xfirst[mask2]
    n=np.size(Zfirst)
    azim=360* np.random.rand(n)*0+180
...
# ATTENTION!!! FACTOR ON RN (removed)
    ...
    for i in range (n):
        ...
        if math.isnan(chi2[i]):
            chi2[i]=1
            print('These are init parameters,\n ',init, 'Fitted params,\n ',popt)
            print('Values of fit to calc chi2 \n',yfit[0:50])
            plt.figure()
            plt.plot(x, y * 1e5, label='Profile, zenith= ' + zen)
            plt.plot(x, GH(x, *popt) * 1e5, '--', markersize=0.5, label='GH fit')
            plt.plot(x, GH(x, *init) * 1e5, '.', markersize=0.5, label='GH initial fit')
            ...
            plt.legend()
            kurs = "%i.png" % i
            plt.savefig('profile_and_fit' + kurs, format='png')
            plt.close()
    ...
    print('Number of events with chi not converging: ', np.size(chi2[chi2 == 1e5]))
    #print(np.array(TauEnergy[chi2 == 1e5]),'All energies', np.array(TauEnergy))
    ...
    plt.figure()
    """
    data_list = [('Nmax \t \t \t \t', Nmx),('ShowerEnergy \t \t \t \t ', (data["tauEnergy"][mask])[mask2]),('lenDec \t \t \t \t', (data["lenDec"][mask])[mask2]), ('lenMax \t \t \t \t', lenMax),('altDec \t \t \t \t \t', (data["altDec"][mask])[mask2]), ('HeightMax \t \t \t \t ', heightMax),  ('Zenith \t \t \t \t ', zenith),
                 ('azimuth \t \t \t', azim)]
