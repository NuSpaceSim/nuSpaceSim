...
# Go up one level to reach the project root (nuSpaceSim/)
project_root = os.path.abspath(os.path.join(base_dir, "..", ".."))

# Build the absolute path to the .npz file
spline_path = os.path.join(project_root, "depth_spline_1414atm.npz")

# Load it
data_loaded = np.load(spline_path)
t = data_loaded["t"]
m = data_loaded["c"]
k = int(data_loaded["k"])
depth_spline = BSpline(t, m, k, extrapolate=True)
...
# Build the absolute path to the .npz file
spline_path = os.path.join(project_root, "height_spline_1414atm.npz")
