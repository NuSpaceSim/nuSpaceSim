    ...
    if plt_type.lower() == "regular":
        ...
        for row in range(0, 1000):
            n_max, x_max, x_0, lambda_at_x_max = conex_macros.gh_param_reader(
                file_name=file_name, data_name=data_name, row=row, x_limit=2000
            )[2:6]
            cutoff_depth = conex_macros.gh_profile_plot(
                file_name=file_name,
                data_name=data_name,
                start_row=row,
                end_row=row,
        ...
        conex_macros.parameter_histogram(
            param=n_maxs, title="Nmax Values ", x_label="Value", color="royalblue"
        )

        conex_macros.parameter_histogram(
            param=x_maxs, title="Xmax Values ", x_label="Value", color="orange"
        )

        conex_macros.parameter_histogram(
            param=x_0s, title=r"$X_{0}$" + " Values ", x_label="Value", color="green"
        )

        conex_macros.parameter_histogram(
            param=lambda_at_x_maxs,
            title="Lambda (t = Xmax) Values ",
            x_label="Value",
            color="purple",
        )

        conex_macros.parameter_histogram(
            param=threshold_depths,
