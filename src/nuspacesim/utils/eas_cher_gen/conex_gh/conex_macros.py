...
def conex_converter(
    input_file, output_file="converted.h5", output_dataset="conex_output"
):
...
def gh_param_reader(
    row, x_limit, showers=None, file_name=None, data_name=None, bins=2000
):
    """
    ...
    x_max = showers[
        row, 5
    ]  # G-H fit result for slant depth of the shower maximum (g/cm^2)
...
def gh_profile_plot(
    ...
    if n_max_cut_threshold is not None:
        ...
        for row in range(start_row, end_row + 1):
            x, f, n_max, x_max = gh_param_reader(
                row=row, x_limit=x_limit, bins=bins, showers=showers
            )[0:4]
            ...
            if places_to_cut[-1] > location_nearest_n_max and f[-2] < f[-1]:
                ...
                trimmed_f = np.pad(
                    trimmed_f,
                    (0, bins - len(trimmed_f)),
                    "constant",
                    constant_values=float("nan"),
                )
                trimmed_x = np.pad(
                    trimmed_x,
                    (0, bins - len(trimmed_x)),
                    "constant",
                    constant_values=float("nan"),
                )
        ...
        if regular_plot is not None:
            ...
            plt.title(
                "G-H Plots | " + str(file_name.split("/")[-1]) + "/" + str(data_name)
            )
            plt.ylabel("Number of Particles N")
            plt.xlabel("Slant Depth t " + "($g \; cm^{-2}$)")
            plt.xlim(left=0)
...
def parameter_histogram(
    param, title, x_label, color=None, hist_type="counts", round_by=0
):
    """
    ...
    if hist_type != "counts":
        ...
        plt.title(
            title
            + "\n[Mean = "
            + str(round(np.mean(param), round_by))
            + ", Median = "
            + str(round(np.median(param), round_by))
            + ", StDev = "
            + str(round(np.std(param), round_by))
            + "]"
        )

        plt.annotate(
            "Total Counts = " + str(len(param)),
            xy=(0.75, 0.96),
