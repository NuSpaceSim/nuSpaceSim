...
def WGS84ellipse(coord):
    a2=(6378137)**2  #earth major axis WGS84
    b2=(6356752.314245)**2 #minor axis
...
def geodetic_to_utm(lat, lon,height):
    ...
    M = (
        a * ((1 - e2 / 4 - 3 * e2**2 / 64 - 5 * e2**3 / 256) * lat_rad
...
def ecef_to_utm(groundecef,height=h):
    ...
    """
    lat, lon, height = ecef_to_latlonh(groundecef)
...
def gen_points(n,r,maxang=np.radians(90)):
    ...
    ninit=int((1+1/3)*1.6*n/np.sin(maxang)) #
    ...
    groundecef, vcoordecefground, beta= ground_xy(coordecef,vcoordecef)
    ...
    return groundecef[mask,:][0:n,:], vcoordecefground[mask,:][0:n,:],beta[mask][0:n]#coordg[mask,:][0:n,:], vcoordg[mask,:][0:n,:]
...
def decay(groundecef,vecef,beta, lgE,earth_radius=earth_radius_centerlat):
    #get decay altitude
    tauEnergy=10**(lgE-9)
