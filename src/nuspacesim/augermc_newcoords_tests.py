...
def ecef_to_enu_matrix(ecef_origin,lat=None,lon=None):
    ...
    # Extract x, y, z from the first point
    x, y, z = ecef_origin[0, 0], ecef_origin[0, 1], ecef_origin[0, 2]
...
def gen_points(n,r,maxang=np.radians(90)):
    ...
    #print(LL+center,enutoecef(centerecef,LL))
    #print('hola', latlongtoECEF(LLlat,LLlong,LLheight), LLlat,LLlong,LLheight,ecef_to_latlonh_grok(LLecef),ecef_to_latlonh(LLecef))
    #exit()
...
def trajectory_inside_tel_sphere_plots(lgE,coordecef,vcoordecef,telphi=telphi,teltheta=teltheta,extraradius=1.01,ntels=telposecef.shape[0]):
    r=Rcutoff(lgE)*extraradius
    eyevector=gen_eye_vectors(telphi,teltheta)
    #eyevector=enutoecef_vector(centerecef,eyevector)
    ##print(eyevector[0,2])
