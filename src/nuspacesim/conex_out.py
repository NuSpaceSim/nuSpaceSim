...
def conex_out(data,config):
    ...
    mask = (Zfirst >= Zmin) & (Zfirst <= Zmax) #&(TauEnergy >= TauEnergyMax) & (data["lenDec"]>=50) & (np.degrees(data["beta_rad"]) >= 5) )
    ...
    with open('showerdata.csv', 'r') as file:       #Read shower profile, slant depth and height.
