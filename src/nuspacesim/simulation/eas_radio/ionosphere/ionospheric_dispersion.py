...
def dispersion_phases(freq, TEC, Bcos_theta=50.e-6):
    alpha = 1.3445e9 # (Hz/TECU)
    em_ratio = 1.759e11 # Hz / Tesla, electron charge to mass ratio
    c = 299792458.
    C2 = alpha * TEC
    C3 = alpha * TEC * 2. * em_ratio * Bcos_theta
    C4a = 3./2. * alpha**2 * TEC**2 * c/(250.e3)
    C4b = 3./2. * C2 * 3. * (Bcos_theta * em_ratio)**2
...
def disperse(wfm_h, wfm_v, fs, TEC, Bcos_theta=50.e-6):
    ...
    fft_w_x[1:] *= np.exp(1j * 2. * np.pi * phase2[1:])
    fft_w_x[1:] *= np.exp(1j * 2. * np.pi * phase3[1:])
    fft_w_x[1:] *= np.exp(1j * 2. * np.pi * (phase4a[1:] + phase4b[1:]))
    
    fft_w_o[1:] *= np.exp(1j * 2. * np.pi * phase2[1:])
    fft_w_o[1:] *= np.exp(-1j * 2. * np.pi * phase3[1:])
    fft_w_o[1:] *= np.exp(1j * 2. * np.pi * (phase4a[1:] + phase4b[1:]))
...
def wfm(f1=30e6,f2=80e6, fs=200e6,delay=3e-6, record_length=4096./200.e6,index=1.):
    dt=1./fs
    ...
    fft_w[i_f1:i_f2]*=np.exp(-1j*2.*np.pi*delay*f[i_f1:i_f2])
...
def noise_fake(f1,f2, fs=200.e6, record_length=4096/200.e6, index=-2.6):
    dt = 1./fs
    N=int(record_length/dt)
    if(N%2!=0): N+=1
    df = fs/N
    w = 0.*np.arange(N, dtype=np.float64)
    n_fft = np.fft.rfft(w)
...
def TEC_search(f1=30., f2=300., TEC=10., Bcos_theta=50.e-6, SNRa=10, Nscan=11, dTEC=0.05, polang=-999):
    ...
    for fname in os.listdir(dirname):
        ...
        Nt = t.shape[0]
        if(polang==-999):
            pang = np.random.uniform(-np.pi/2., np.pi/2.)
        ...
        tTEC_list = np.arange(TEC-Nscan/2.*dTEC, TEC+(Nscan/2.+1e-7)*dTEC, dTEC)
        if(Nscan == 1):
            tTEC_list = [TEC]
        ...
        for tTEC in tTEC_list:
            ...
            fft_wX = np.exp(-1j*2*np.pi*(ph2+ph3+ph4b))/np.sqrt(2.)
            fft_wO = np.exp(-1j*2*np.pi*(ph2-ph3+ph4b))/np.sqrt(2.)
            wX=np.fft.irfft(fft_wX,Nt)
            wO=np.fft.irfft(fft_wO,Nt)
            
            wP = np.fft.irfft((fft_wX + fft_wO), Nt)/np.sqrt(2.)
            wM = np.fft.irfft(-1j*(fft_wX - fft_wO), Nt)/np.sqrt(2.)
            ...
            v2.append(np.max(np.sqrt(corr_Ph**2 + corr_Mh**2 + corr_Pv**2 + corr_Mv**2)))
            #plt.figure(22)
...
def bfield_search(f1=30., f2=300., TEC=10., Bcos_theta=50.e-6, SNRa=10, Nscan=11, dBF=0.05, polang=-999):
    dirname = '/home/abl/nuspacesim/src/nuspacesim/simulation/eas_radio/zhaires/wfms/'
    frange = (f1, f2)
    for fname in os.listdir(dirname):
        zenith = int(fname.split('_')[3])
        ...
        w3_h = SNRa*w3_h + wn_h
        w3_v = SNRa*w3_v + wn_v
        bfield_list = np.arange(Bcos_theta-Nscan/2.*dBF, Bcos_theta+(Nscan/2.+1e-9)*dBF, dBF)
        ...
        for B in bfield_list:
            ...
            v2.append(np.max(np.sqrt(corr_Ph**2 + corr_Mh**2 + corr_Pv**2 + corr_Mv**2)))
            plt.figure(22)
            plt.plot([B*1e6], [np.max(np.sqrt(corr_Ph**2 + corr_Mh**2 + corr_Pv**2 + corr_Mv**2))], 'go', mfc='none')
...
def TEC_search_fakewf(f1=30., f2=300., TEC=10., Bcos_theta=50.e-6, SNRa=10, Nscan=11, dTEC=0.05, polang=-999):
    fs = 200e6
    t, subS = wfm(f1, f2)
    norm = np.max(np.abs(subS))
    subS *= SNRa/norm
    ...
    w3_h = w3_h + wn_h
    w3_v = w3_v + wn_v
    tTEC_list = np.arange(TEC-Nscan/2.*dTEC, TEC+(Nscan/2.+1e-3)*dTEC, dTEC)
    if Nscan == 1:
        tTEC_list = [TEC]
    v1=[]
    v2=[]
    ...
    for tTEC in tTEC_list:
        N = len(np.fft.rfft(w3_h))
        df = fs/N
        freq = np.arange(0., N, 1., dtype=np.float64) * df
...
def ionospheric_dispersion(flow=30.e6, fhigh=300.e6, TEC=10., Bcos_theta = 50.e-6):
    dirname = '/home/abl/nuspacesim/src/nuspacesim/simulation/eas_radio/zhaires/wfms/'
    frange = (flow, fhigh)
    for fname in os.listdir(dirname):
        zenith = int(fname.split('_')[3])
        ...
        plt.pcolormesh(t, f, np.log10(Sxx), vmin=np.log10(np.max(Sxx))-2.)
        plt.ylabel('Freq (Hz)')
        plt.xlabel('Time')
        plt.figure()
...
def generate_TEC_files(f1, f2, TECerr):
    ...
    ntimes = 5
    ns = 51
    dt = (2.*TECerr)/float(ns)
    funcs = np.array([])
    tecs = np.array([])
    for TECu in [1., 5., 10., 50., 100., 150.]:
        for i in range(0,ntimes):
            if i == 0:
                teclist, v2 = TEC_search(f1=f1*1.e6, f2=f2*1.e6, TEC=TECu, Bcos_theta=42e-6, 
                        dTEC=dt, Nscan=ns, SNRa = 100, polang=-999)
            else:
                tl, vv2 = TEC_search(f1=f1*1.e6, f2=f2*1.e6, TEC=TECu, Bcos_theta=42e-6,
                        dTEC=dt, Nscan=ns, SNRa = 100, polang=-999)
                v2 += vv2
        v2 /= float(ntimes)
    ...
    param_dir = str(importlib_resources.files("nuspacesim.simulation.eas_radio.ionosphere.tecparams"))
...
def perfect_TEC_disperse(f1=30., f2=300., TEC=10., Bcos_theta=50.e-6, SNRa=100, polang=-999):
    dirname = '/home/abl/nuspacesim/src/nuspacesim/simulation/eas_radio/zhaires/wfms/'
    frange = (f1, f2)
    for fname in os.listdir(dirname):
        zenith = int(fname.split('_')[3])
