...
from .ionosphere.ionospheric_dispersion import perfect_TEC_disperse, generate_TEC_files
...
class RadioEFieldParams(object):
    ...
    def __init__(self, freqRange):
        ...
        """
        param_dir = str(importlib_resources.files("nuspacesim.simulation.eas_radio.zhaires.params"))
...
class IonosphereParams(object):
    ...
    def __init__(self, freqRange, TECerr, TEC):
        ...
        if not self.params_exist:
            print('\n***** WARNING *****')
            print('The only supported parameters for ionospheric dispersion are:')
            print('frequency ranges of 30-80 MHz, 30-300 MHz, 300-1000 MHz')
            print('TECerrs of 0.001, 0.01, 0.1, 1')
            print('TEC values of 1, 5, 10, 50, 100, 150')
            print('Arbitrary values of these parameters will eventually be enabled, but not yet !!')
