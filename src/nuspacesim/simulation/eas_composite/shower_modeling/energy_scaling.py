...
for angle_idx, a in enumerate(sorted(list(set(angles)))):
    ...
    ax[angle_idx].set(xlabel="$\log_{10}$ Energy (eV)", ylim=(1, 15))
...
plt.savefig("./E_scaling_{}.pdf".format(tup_folder.split("/")[-1]))
#%% save
...
with as_file(
    files("nuspacesim.data.eas_scaling_tables.energy_scaling") / f"{fname}.h5"
) as path:
    print(path)
