...
tup_folder = "/home/fabg/g_drive/Research/NASA/Work/conex2r7_50-runs/downward"
...
if plot_type == "lin_log_xmax_vs_energy":
    ...
    fig, ax = plt.subplots(
        nrows=int(len(sorted(list(set(angles)))) / 2),
        ncols=2,
        sharex=True,
        sharey=True,
        figsize=(10, 2 * len(sorted(list(set(angles))))),
        dpi=300,
    )
    ax = ax.ravel()
    ...
    plt.savefig("./elongation_rate_downward.pdf")
