from nuspacesim.simulation.eas_optical.atmospheric_models import cummings_atmospheric_density, us_std_atm_density, slant_depth
import numpy as np
...
plt.figure(figsize=(12, 6), dpi=200)
angles = np.radians(np.linspace(0, 80, 8)) 
...
for angle in angles:
    ...
    plt.scatter(altitudes[1:], slt_dpths[:,:1], label='{:.2f} degrees'.format(np.degrees(angle)))
